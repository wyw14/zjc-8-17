...
function initDreams() {
  ...
  const needsUpdate = dreams.length > 0 && dreams.some(d => d.isPrivate === undefined);
  if (needsUpdate) {
    const updated = dreams.map(d => ({ ...d, isPrivate: false }));
    writeJSON(DREAMS_FILE, updated);
    return;
  }
  if (dreams.length === 0) {
    const sampleDreams = [
...
app.post('/api/dreams', authenticateToken, (req, res) => {
  const { content, lucidity, date, isPrivate } = req.body;
...
app.put('/api/dreams/:id', authenticateToken, (req, res) => {
  ...
  const { content, lucidity, date, isPrivate } = req.body;
  const dreams = readJSON(DREAMS_FILE);
  const dreamIndex = dreams.findIndex(d => d.id === parseInt(id) && d.userId === req.user.id);
  ...
  if (lucidity !== undefined) dream.lucidity = parseInt(lucidity);
  if (date !== undefined) dream.date = date;
  if (isPrivate !== undefined) dream.isPrivate = isPrivate ? true : false;

  dreams[dreamIndex] = dream;
  writeJSON(DREAMS_FILE, dreams);
  res.json(dream);
});
